...
FEAT-031 collapses the repo's two divergent test trees (TS under `scripts/__tests__/`, Bats under `plugins/lwndev-sdlc/.../tests/`) into a single `tests/` root with runner-scoped leaves (`tests/unit/` for Vitest, `tests/bats/` for Bats), unifies `npm test` to drive both runners, and lands a validator + PreToolUse hook so misplaced test files cannot regress the layout. The migration ships in seven phases that mirror NFR-6's commit-per-phase guidance: Vitest relocation -> Bats relocation -> config/npm-script updates -> per-skill fixture relocation -> validator -> hook -> doc alignment + payload verification.

The build order is sequenced for review/revert isolation: each relocation phase is a mechanical move (with documented sed rewrites for Bats path resolution per FR-3) that keeps the existing test suite green at every commit boundary; the config phase (Phase 3) flips Vitest's `testMatch` and adds `tsconfig.test.json` / ESLint / `npm test` wiring **after** the files have already moved, so no file is ever orphaned from a runner; the per-skill fixture relocation (Phase 4) closes the FR-12 spirit gap that surfaced after Phase 2 — `.bats`/`.test.ts`/`.spec.ts` moved out of `plugins/` but the `.md`/`.json`/`.txt` fixtures those tests consume stayed behind and would still ship in `/plugin install` payloads — and is run as a single commit so partial-move state never breaks the per-skill `${BATS_TEST_DIRNAME}` rewrites; the validator (Phase 5) and hook (Phase 6) land **after** the layout is correct, so they do not block their own enabling commits; doc alignment + plugin-install payload verification (Phase 7) closes the loop. The shared `scripts/test-layout-rules.ts` module (Edge Case 5) lands in Phase 5 alongside the validator and is consumed by the hook in Phase 6 — single source of truth for both tools' allow-rule and rule IDs.

Phases 1, 2, 3, 4, 5, and 7 carry `**ComplexityOverride:** opus` clamps by intent: NFR-6 mandates one commit per phase boundary (Vitest relocation; Bats relocation; config + npm-script updates; per-skill fixture relocation; validator; hook; doc alignment) so the relocation is reviewable and revertible as a single unit per logical step. Splitting any of these phases would violate NFR-6 — Phase 1 must move all 51 TS files in one commit (otherwise `vitest.config.ts` `testMatch` flips against a partially-moved tree), Phase 2 must move all 68 Bats files in one commit (otherwise `npm test:bats` flips against a partial set), Phase 3 must land the five interlocking config edits together (otherwise `npm test`, ESLint, and `tsconfig.test.json` are mutually inconsistent at intermediate commits), Phase 4 must move all four per-skill fixture trees and rewrite both the 21 Bats `FIXTURES=` assignments and the 5 Vitest hardcoded paths in one commit (otherwise either the Bats or the Vitest side temporarily resolves to a missing path), Phase 5 must land the validator and its rule module together (otherwise the validator has no allow-rule to import), and Phase 7 must land the doc audit and the FR-12 install verification together (otherwise the verification runs against a tree with stale doc guidance). Phase 6 (hook) stays within the per-phase budget without a clamp.
...
- [x] `vitest.config.ts` (final `testMatch`, updated `coverage.include`/`coverage.exclude`)
- [x] `tsconfig.test.json` (new, extends base)
- [x] `eslint.config.js` (references both tsconfigs)
- [x] `package.json` (added `bats` devDep, added `test:unit`/`test:bats`, rewrote `test`, widened `lint`/`format`/`lint-staged` globs)
- [x] `package-lock.json` (regenerated by `npm install`)
...
Phase 2 moved all 68 `.bats` files out of `plugins/lwndev-sdlc/.../tests/` but Phase 2's report explicitly noted "four skill `tests/` dirs (creating-implementation-plans, implementing-plan-phases, orchestrating-workflows, reviewing-requirements) retained because their `fixtures/` subdirectories aren't part of the move." The result is 40 fixture files (~156 KB of `.md`/`.json`/`.txt`) still living under `plugins/lwndev-sdlc/skills/<skill>/scripts/tests/fixtures/` after Phase 3 lands. These fixtures are contributor-only artifacts (no runtime code consumes them) yet they ship in the `/plugin install` payload — a direct violation of the user-story commitment "the installed plugin contains only the runtime files I will execute." FR-12's literal `find` clause checks only `*.bats`/`*.test.ts`/`*.spec.ts`, so fixture residue slips through verification unless the migration is extended to cover them.

A second motivation: `budget-mixed-plan.md` is referenced from **both** Vitest (5 hardcoded paths in `tests/unit/{qa-feat-029,orchestrating-workflows,workflow-state}.test.ts`) and Bats (`tests/bats/shared/prepare-fork.bats:53` plus 5 references in two skill bats files). With tests now under `tests/` and fixtures still under `plugins/`, there is no canonical tests-tree home for cross-runner shared state. Phase 4 closes that gap by mirroring FR-2's `qa-fixture/` precedent — fixtures land alongside the tests that consume them under `tests/fixtures/<skill>/` (parallel layout to `tests/fixtures/qa-fixture/` already established by Phase 1).

The phase also drops four orphaned fixtures discovered during the audit (zero references in any test): `split-overflow-plan.md` (created by FEAT-029 for a test that was never written), and three `sdlc-workflows/` JSON files (`BUG-001.json`, `CHORE-001.json`, `CHORE-002-pre-feat030.json`) that have `seed_state` infrastructure support but no caller. `FEAT-030-qa.json` is **kept** despite grep missing it as a literal string — `tests/bats/skills/orchestrating-workflows/workflow-state-record-findings-qa.bats:39` calls `seed_state "FEAT-030-qa"` which dynamically resolves to that filename.

The phase's scope sits between layout-config (Phase 3) and the validator (now Phase 5) because the validator's clean-tree assumption only requires `.test.ts`/`.spec.ts`/`.bats` to be at canonical leaves — fixture `.md`/`.json`/`.txt` files are outside the validator's rule surface. Landing fixture migration here closes FR-12's spirit gap before the install-verification step in Phase 7, and keeps the per-phase budget within NFR-6 (one mechanical move + scoped path rewrites per skill).

#### Implementation Steps

1. Create `tests/fixtures/creating-implementation-plans/`, `tests/fixtures/orchestrating-workflows/`, `tests/fixtures/reviewing-requirements/`, and `tests/fixtures/implementing-plan-phases/`. The `tests/fixtures/qa-fixture/` tree is already in place from Phase 1.
2. `git mv` the four per-skill `fixtures/` trees into the new layout. Preserve subdirectory nesting under `orchestrating-workflows/` (the `qa-artifacts/` and `sdlc-workflows/` subdirs move along with their files). Use `git mv` so blame survives. Total: 36 files after orphan deletion (10 - 1 + 20 - 3 + 5 + 5).
3. Delete the 4 orphans during the move (do not move them to the new location): `creating-implementation-plans/scripts/tests/fixtures/split-overflow-plan.md`, `orchestrating-workflows/scripts/tests/fixtures/sdlc-workflows/{BUG-001,CHORE-001,CHORE-002-pre-feat030}.json`. Confirm zero test references via `grep -rn split-overflow-plan tests/` and `grep -rn 'BUG-001\|CHORE-001\|CHORE-002-pre-feat030' tests/` before deletion. Keep `FEAT-030-qa.json` (referenced via `seed_state "FEAT-030-qa"`).
4. Apply scoped sed rewrites to 21 Bats skill-leaf files. Pattern: `FIXTURES="${BATS_TEST_DIRNAME}/../../../../plugins/lwndev-sdlc/skills/<skill>/scripts/tests/fixtures"` -> `FIXTURES="${BATS_TEST_DIRNAME}/../../../fixtures/<skill>"`. The 21 files live under `tests/bats/skills/<skill>/`, so `${BATS_TEST_DIRNAME}` resolves to `tests/bats/skills/<skill>/` — three `..` levels reach `tests/`, then `fixtures/<skill>/`. Iterate per-skill so the literal `<skill>` segment stays scoped per pass. Apply the same rewrite to the special-case `FIXTURE_SRC=` line in `tests/bats/skills/implementing-plan-phases/verify-phase-deliverables.bats:18`.
5. Rewrite the shared-leaf reference at `tests/bats/shared/prepare-fork.bats:53`: `BUDGET_MIXED_PLAN="${PLUGIN_ROOT}/skills/creating-implementation-plans/scripts/tests/fixtures/budget-mixed-plan.md"` -> `BUDGET_MIXED_PLAN="${BATS_TEST_DIRNAME}/../../fixtures/creating-implementation-plans/budget-mixed-plan.md"`. The shared-leaf `${BATS_TEST_DIRNAME}` resolves to `tests/bats/shared/`, so 2× `..` reaches `tests/`. Drop the `PLUGIN_ROOT` indirection — helper-relative paths are clearer.
6. Rewrite 5 Vitest hardcoded paths to `budget-mixed-plan.md` in 3 files: `tests/unit/qa-feat-029.test.ts:18`, `tests/unit/orchestrating-workflows.test.ts:1079, 1358`, `tests/unit/workflow-state.test.ts:1665, 1967, 2241`. Replace `'plugins/lwndev-sdlc/skills/creating-implementation-plans/scripts/tests/fixtures/budget-mixed-plan.md'` with `'tests/fixtures/creating-implementation-plans/budget-mixed-plan.md'` (relative to repo root, matching how `tests/unit/qa-{dependency-failure,inputs}-BUG-012.test.ts` resolve `tests/fixtures/qa-fixture/` via `join(ROOT, ...)`).
7. Delete the now-empty per-skill `tests/` parent directories (`plugins/lwndev-sdlc/skills/{creating-implementation-plans,orchestrating-workflows,reviewing-requirements,implementing-plan-phases}/scripts/tests/`). `git status` should show only file moves and the directory deletions.
8. Run `npm test 2>&1 | tee /tmp/feat-031-phase4-test.log` to confirm both runners green: Vitest count unchanged (1548), Bats count unchanged (1077 pass + 6 pre-existing record-findings-qa.bats failures from main). Run `npm run validate` to confirm no regressions. Run `find plugins/lwndev-sdlc -path '*/scripts/tests*'` and confirm empty output (no test residue under `plugins/`). Run `find plugins/lwndev-sdlc -name '*.md' -path '*fixtures*' -not -path '*/.claude-plugin/*'` and confirm empty (no fixture residue).
9. Commit with message `refactor(tests): relocate per-skill fixtures to tests/fixtures/<skill>/ (FEAT-031 phase 4)`.
...
Phase 5 lands the validator that prevents future regression of the Phase 1-4 layout. The validator is a TS script (`scripts/validate-test-layout.ts`) that walks the working tree and emits a normative-rule-keyed line per violation. The rule IDs (`ts-outside-tests-unit`, `spec-extension-disallowed`, `bats-outside-tests-bats`) and the single allow-rule path (the `feat-030-known-buggy` fixture's `.spec.ts`) live in `scripts/test-layout-rules.ts` so the validator and Phase 6's hook share one source of truth (Edge Case 5 — split state would let one tool block work the other generates).

Wiring the validator into `npm run validate` and `.husky/pre-commit` happens in this phase: `package.json`'s `validate` script becomes `tsx scripts/validate-test-layout.ts && tsx scripts/build.ts` so a layout failure short-circuits before manifest validation, and `.husky/pre-commit` appends `npm run validate` after the existing chain (FEAT-031 inventory rationale: `lint-staged` scopes to staged files only; the validator must scan the whole tree). Phase 5 depends on Phase 4 because the validator's clean-tree path requires the post-Phase-4 layout already in place — running the validator on a pre-relocation tree would fire on every existing test file, and running it before Phase 4 would still pass (the validator's three rules don't cover fixture residue) which would falsely declare success.
...
8. Commit with message `feat(tests): add layout validator + shared rule module (FEAT-031 phase 5)`.
...
5. Run `npm test`, then `npm run validate` to confirm zero regressions. Confirm the validator (Phase 5) and hook (Phase 6) both share the rule module — grep `import` lines to verify both files import from `scripts/test-layout-rules.ts`.
6. Commit with message `feat(tests): add PreToolUse layout-enforcement hook (FEAT-031 phase 6)`.
...
Phase 7 closes the loop on documentation and verifies the plugin-install payload is clean. With code, configs, fixtures, validator, and hook all landed, the docs that previously pointed contributors at the old layout (`scripts/__tests__/`, `.spec.ts` patterns, `__tests__/qa-<dimension>.spec.ts (or .test.ts)`) must be rewritten to the new layout — otherwise the next contributor reads stale guidance and authors a misplaced test that the validator and hook then block, causing avoidable churn. CLAUDE.md gets the §3 Skill Authoring update; `executing-qa/SKILL.md` gets the line ~178 rewrite to `tests/unit/qa-<dimension>.test.ts`; every other `SKILL.md` is audited for `__tests__/`, `.spec.ts`, or stale `plugins/lwndev-sdlc/skills/.../scripts/tests/fixtures/` path mentions.

Plugin-install payload verification (FR-12) ships in this phase as a manual acceptance step. Two `find` commands together establish payload cleanliness: `find <plugin-install-path> -type f \( -name '*.bats' -o -name '*.test.ts' -o -name '*.spec.ts' \)` returns empty (test-file residue check, the original FR-12 clause), and `find <plugin-install-path> -type d -name fixtures` returns empty (fixture-residue check, added because Phase 4's fixture migration is the load-bearing step that closes this gap). Together they are the NFR-5 payload-reduction success metric. The marketplace `source` path already points at `./plugins/lwndev-sdlc`, and Phases 2 and 4 emptied that subtree of tests and fixtures respectively, so no `marketplace.json` edit is required (Edge Case 7); the install verification is the proof that Phases 2 and 4 actually achieved payload reduction. Contributor `npx bats <path>` documentation also lands here per FR-7.

#### Implementation Steps

1. Update `CLAUDE.md` "Skill Authoring: Prefer Scripts Over Prose" section §3 ("Every script behavior needs bats coverage") to read: "TS modules -> Vitest under `tests/unit/`; shell scripts -> Bats under `tests/bats/{shared,skills/<skill>}/`; shared fixtures under `tests/fixtures/<skill>/` or `tests/fixtures/qa-fixture/`. New test files land at the canonical leaf or the validator/hook (Phase 5/6 of FEAT-031) blocks the commit." Add a "Running tests locally" subsection (or update the existing `npm test` documentation) to include `npx bats tests/bats` for shell-only iteration and reference the contributor-install path of the `bats` npm devDep (FR-7).
...
3. Audit every other `SKILL.md` and `references/` doc under `plugins/lwndev-sdlc/skills/` for stale path mentions: `grep -rn '__tests__\|\.spec\.ts\|/scripts/tests/fixtures/' plugins/lwndev-sdlc/skills/`. For each hit, update to the new layout (`tests/unit/`, `tests/bats/`, `tests/fixtures/`, `.test.ts`) or remove the stale mention. Specific files to check (non-exhaustive): `documenting-qa/SKILL.md`, `executing-bug-fixes/SKILL.md`, `reviewing-requirements/SKILL.md`, and any `references/` docs that mention test paths.
4. Run `npm test`, then `npm run validate` to confirm the doc edits do not break plugin validation.
5. Manually verify the plugin-install payload (FR-12, NFR-5): on a fresh Claude config (or `CLAUDE_CONFIG_DIR` override pointing to a temporary directory), run `/plugin marketplace add lwndev/lwndev-marketplace` and `/plugin install lwndev-sdlc@lwndev-plugins`; then run BOTH verification clauses and confirm BOTH return empty: `find <plugin-install-path> -type f \( -name '*.bats' -o -name '*.test.ts' -o -name '*.spec.ts' \)` (test-file residue) and `find <plugin-install-path> -type d -name fixtures` (fixture-residue, gating Phase 4's payload-reduction outcome). Capture the `du -sh <plugin-install-path>` before/after numbers for the PR description (NFR-5). Document both verification commands and results in the PR body.
...
8. Commit with message `docs(tests): align skill docs with new tests/ layout (FEAT-031 phase 7)`.
...
- `scripts/test-layout-rules.ts` (Phase 5) is the single source of truth for rule IDs (`ts-outside-tests-unit`, `spec-extension-disallowed`, `bats-outside-tests-bats`) and the `feat-030-known-buggy` fixture allow-rule. Both `scripts/validate-test-layout.ts` (Phase 5) and `scripts/hooks/validate-test-layout-hook.ts` (Phase 6) import from this module — Edge Case 5's split-state risk is eliminated by direct reuse.
- `tests/fixtures/` (Phases 1 + 4) is the single canonical home for shared test assets. After Phase 4 it contains: `qa-fixture/` (Phase 1, Vitest harness fixture), `creating-implementation-plans/` / `orchestrating-workflows/` / `reviewing-requirements/` / `implementing-plan-phases/` (Phase 4, per-skill fixtures consumed by skill-leaf Bats and select Vitest tests). `budget-mixed-plan.md` under `creating-implementation-plans/` is the only cross-runner shared file; both Vitest and Bats reference it from this single location.
...
**Unit (Vitest)**: Phase 5 ships `tests/unit/validate-test-layout.test.ts` covering the validator's clean-tree, three-rule, allow-rule, and exit-code paths. All 51 existing TS tests continue to run from `tests/unit/` after Phase 1, with the 5 hardcoded `budget-mixed-plan.md` references repointed to `tests/fixtures/creating-implementation-plans/budget-mixed-plan.md` after Phase 4.

**Unit (Bats)**: Phase 6 ships `tests/bats/shared/hooks/validate-test-layout-hook.bats` covering the hook's three reject paths, allow-rule, non-test-no-op path, and Edge Case 4 fail-open. All 68 existing Bats tests continue to run from `tests/bats/` after Phase 2 (and now run via `npm test` after Phase 3 — first time pre-commit / CI exercises them). After Phase 4, the 21 skill-leaf `FIXTURES=` constants and the shared `prepare-fork.bats:53` reference resolve to `tests/fixtures/<skill>/` rather than to `plugins/lwndev-sdlc/.../scripts/tests/fixtures/`.

**Integration**: Phase 7 manually verifies `npm test` on a fresh checkout exits 0 with both runners executing, `npm run validate` passes including layout validator, and the PreToolUse hook fires correctly against synthetic `tool_input.file_path` payloads (the bats suite covers the hook unit-side; the manual smoke test covers the live wiring).

**Acceptance (NFR-1, NFR-4, NFR-5)**: Phase 1 captures the pre-feature Vitest baseline on `main` for NFR-1 zero-loss verification. Phase 7 captures the post-feature counts and runtimes for the comparison. The plugin-install payload verification (FR-12, both `find` clauses including the fixture-residue check added in Phase 4) is the NFR-5 acceptance proof.
...
| Phase 4 fixture-path rewrite misses a non-literal reference (e.g., a TS file that builds the fixture path with `join(...)` rather than a literal string, or a Bats helper that resolves the path through a layered variable) | High | Low | Phase 4 step 4-6 prescribes literal-string sed/Edit operations across a fixed audit (21 Bats `FIXTURES=` lines + 1 `prepare-fork.bats` shared-leaf line + 5 Vitest hardcoded paths in 3 files). Step 8 runs `npm test` and `bats tests/bats` immediately after the rewrite, so any missed reference fails fast at test discovery. The orphan-deletion grep audit in step 3 also surfaces dynamic resolutions like `seed_state` callers. If a layered helper slips through, fix it in the same Phase 4 commit before pushing. |
...
| Plugin-install payload verification (FR-12) fails because a `.test.ts`, `.spec.ts`, or `fixtures/` directory slipped under `plugins/` post-feature | High | Low | The Phase 5 validator and Phase 6 hook both block test-file regression at commit and at runtime. Phase 4 closes the fixture-residue gap (the validator's three rules don't cover `.md`/`.json`/`.txt`). Phase 7 step 5 runs both `find` clauses (test-file residue and fixture-residue) as the explicit empirical check; if either fails, fix the offending placement before merging. |
...
- Phase 2: All 68 `.bats` files moved to `tests/bats/{shared,shared/hooks,skills/<skill>}/`; `BATS_TEST_DIRNAME`-anchored expressions rewritten per FR-3 replacement table; `bats tests/bats` passes; old `tests/` directories under `plugins/` deleted (except those still containing `fixtures/`, addressed in Phase 4); `npm test` (Vitest still alone) clean; `npm run validate` clean.
...
- Phase 4: All 4 per-skill `fixtures/` trees relocated to `tests/fixtures/<skill>/` (`creating-implementation-plans/`, `orchestrating-workflows/`, `reviewing-requirements/`, `implementing-plan-phases/`); 21 Bats `FIXTURES=`/`FIXTURE_SRC=` assignments rewritten plus the 1 shared-leaf reference in `prepare-fork.bats`; 5 Vitest hardcoded `budget-mixed-plan.md` paths rewritten in `qa-feat-029.test.ts`/`orchestrating-workflows.test.ts`/`workflow-state.test.ts`; orphans deleted (`split-overflow-plan.md` + `BUG-001.json`/`CHORE-001.json`/`CHORE-002-pre-feat030.json`); 4 per-skill `scripts/tests/` parent directories removed; `npm test` and `bats tests/bats` green at the post-Phase-3 baseline; `find plugins/lwndev-sdlc -path '*/scripts/tests*'` returns empty.
- Phase 5: `scripts/test-layout-rules.ts` exports rule IDs and `ALLOWED_FIXTURE_PATHS`; `scripts/validate-test-layout.ts` emits the FR-9 output format and exits non-zero on violations; `npm run validate` chains layout validator before build validator; `.husky/pre-commit` invokes `npm run validate` after `npm audit`; misplacement smoke test confirms the validator fires correctly.
