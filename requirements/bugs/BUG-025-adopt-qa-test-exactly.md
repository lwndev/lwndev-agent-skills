...
- [x] The genuine no-peer case (zero resolvable peers, `multi_seen=0` and `seen_count=0`) still exits 2 with the existing `no existing peer test found` / `no existing peer .bats test found` reason. The `<<MULTI>>`-with-no-singular-peer case (`multi_seen=1` and `seen_count=0`) exits 2 with a multi-peer-specific reason (`multiple plausible peer tests match a single imported SUT; cannot disambiguate`) — peers exist but are ambiguous for that SUT, so "no existing peer test found" would misdirect at the FR-5 pause UI. Only multi-peer-with-at-least-one-resolved ambiguity changes to exit 0. (RC-1, RC-2; revised per PR #312 review)
- [x] The exit-code contract comment in `adopt-qa-test.sh` header is updated: "multiple plausible peers" across distinct SUTs is removed from the exit-2 reasons (now tolerated and picked), while the narrow single-SUT-ambiguous-peer case is enumerated as an exit-2 reason (RC-1, RC-2; revised per PR #312 review)
- [x] `tests/bats/skills/addressing-qa-findings/adopt-qa-test.bats` covers the multi-SUT vitest/jest case, the `<<MULTI>>` parallel-root case (singular peer from another import wins), the multi-load bats case, the unchanged no-peer failure, and the `<<MULTI>>`-with-no-singular-peer case asserting exit 2 + the multi-peer-specific reason verbatim; the original test asserting exit 2 for multi-SUT imports is rewritten to assert the new exit-0 + lexicographically-first sibling behavior (RC-1, RC-2)
