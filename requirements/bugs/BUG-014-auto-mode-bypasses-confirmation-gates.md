...
- Implementation must preserve the orchestrator-side ability to fork `implementing-plan-phases` with a `Skip Step 10` instruction — that is the one legitimate carve-out (the orchestrator owns PR creation as a separate fork step `5+N+1`). Hook C's regex must whitelist this case.
