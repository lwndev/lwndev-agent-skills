...
| `executing-bug-fixes` | `SKILL.md` | 212 | 1821 | 12845 | 3211 |
| `executing-bug-fixes` | `references/workflow-details.md` | 338 | 1365 | 8649 | — |
| `executing-bug-fixes` | `assets/pr-template.md` | 180 | 878 | 5825 | — |
| `executing-bug-fixes` | **subtotal** | **730** | **4064** | **27319** | — |
| `documenting-qa` | `SKILL.md` | 200 | 1898 | 13354 | 3339 |
| `documenting-qa` | `assets/test-plan-template-v2.md` | 65 | 334 | 2252 | — |
| `documenting-qa` | `assets/test-plan-template.md` | 65 | 422 | 2575 | — |
| `documenting-qa` | **subtotal** | **330** | **2654** | **18181** | — |
...
**Per-axis outcomes (Phase 9, `executing-bug-fixes`):**
- **FR-1**: lite-style prose compression applied to `# Executing Bug Fixes` intro (unchanged, dense), `## When to Use This Skill` bullets (collapsed `Continuing bug fix work that was previously started` to `Continuing previously-started bug fix work` and minor verb-prefix tightening; preserved skill-name literals `executing bug fix`, `documenting-bugs` reference, and `requirements/bugs/` literal), `## Arguments` bullets (preserving the literal `When argument is provided` / `When no argument is provided` phrases verbatim per cross-skill `argument-hint.test.ts`), `## Quick Start` (steps 1-11 phrasings tightened; exit-code documentation, all script names, the `--closes #N` literal, and the `slugify.sh` stopword list / 4-token cap retained verbatim), `## Root Cause Driven Execution` Workflow steps 1-4 and Discovering New Root Causes bullets (compressed prose while preserving `Redeclare root causes`, `Address root causes systematically`, and `Verify per root cause` literals required by `executing-bug-fixes.test.ts`), `## Branch Naming` and `## Commit Message Format` paragraphs (script-call references compressed; bullet wording tightened), `## Verification Checklist` items (added articles for natural reading), and the `## Relationship to Other Skills` trailing chain summary unchanged. The `## Output Style` section (lines 77-111) was preserved verbatim per FR-4 (FEAT-023 carve-out for forked skills) — only the FR-14 carve-out bullet's "Unicode `→`" reference was corrected from the self-contradictory "Unicode `->`" form (Phase 9 carve-out fidelity fix; canonical form per FEAT-014). The `## Workflow Checklist` fenced code block (with the literal `→` arrow in the acceptance-criterion checkoff line and `status → Completed` line) was preserved verbatim.
- **FR-2**: no-op. No non-dispatcher section in SKILL.md exceeded ~25 lines of procedural narrative. The `## Output Style` section (~35 lines) is doctrine/rules (FR-4 preservation target), not procedural narrative subject to relocation. The existing `references/workflow-details.md` (338 lines — slightly larger than the chores sibling) already holds the Phase 1 / Phase 2 / Phase 3 detailed workflow procedure; SKILL.md correctly dispatches to it via the inline pointer at the end of the `## Workflow Checklist` section (`See [references/workflow-details.md](references/workflow-details.md) for detailed guidance on each step.`) and via the `## References` block — pointer placement is at the end of the dispatcher block (Edge Case 8 compliant). No new files added under `references/`.
- **FR-3**: no-op. SKILL.md contains two tables: the `## Commit Message Format` six-row category-to-use-when matrix and the `## Relationship to Other Skills` five-row task-to-approach matrix. Both have distinct shapes and purposes (commit-category→use-case vs task-type→recommended-skill); no repetition or near-duplication exists between them. Nothing to collapse.
- **FR-4**: verified. All carve-outs intact: `## Output Style` section preserved verbatim including (a) `### Lite narration rules` (no-preamble, no-emoji, ASCII-arrows-in-skill-prose clause), (b) `### Load-bearing carve-outs` (error messages from `fail` calls including `resolve-requirement-doc.sh`, `build-branch-name.sh`, `ensure-branch.sh`, `check-acceptance.sh`, `commit-work.sh`, `create-pr.sh` failures plus `npm test` / `npm run build` / `npm run lint` failing output; security-sensitive warnings; interactive prompts including the disambiguation, description re-prompt on empty slug, and selection prompt cases; FR-14 echo lines now in canonical "Unicode `→`" form per the Phase 9 carve-out fidelity fix; tagged structured logs `[info]` / `[warn]` / `[model]`; user-visible state transitions), (c) `### Fork-to-orchestrator return contract` (bug chain step 4, `done | artifact=<path> | <note>` shape, `failed | <one-sentence reason>` shape, the `Found **N errors** ...` exclusion clause for `reviewing-requirements`, the `done | artifact=requirements/bugs/BUG-NNN-*.md | PR #<N> created` example), and (d) the **PR creation is this skill's responsibility** carve-out (the explicit "Step 10 runs in both orchestrated and standalone invocations" clause that distinguishes this skill from `implementing-plan-phases`) and the `Precedence` note. All FR-7 must-preserve items confirmed: `## When to Use This Skill`, `## Quick Start`, `## Verification Checklist`, `## Relationship to Other Skills` headings; literal phrases `documenting-bugs`, `fix/BUG-XXX`, `Closes #N`, `managing-work-items`, `Issue tracking (start/completion comments) is handled by the orchestrator`, `fix(category):`, `Root Cause Driven Execution`, `Redeclare root causes`, `Address root causes systematically`, `Verify per root cause`; PR-template assertions (`Root Cause(s)`, `How Each Root Cause Was Addressed`, `| RC | Fix Applied | Files Changed |`, `RC-N acceptance criteria verified`, `Closes #N`); workflow-details Phase 1/2/3 headings and `[Rr]edeclare [Rr]oot [Cc]ause` / `reproduction steps.*no longer` matchers; the `not.toContain('github-templates.md')` exclusion guard (managing-work-items extraction artifact, no `github-templates.md` reference appears in SKILL.md); `allowed-tools` frontmatter (Read/Write/Edit/Bash/Glob/Grep, no Agent); `argument-hint:`.
- **Cross-skill carve-out (confirmed)**: the `## Arguments` bullets retain `When argument is provided` / `When no argument is provided` verbatim — required by `scripts/__tests__/argument-hint.test.ts` (executing-bug-fixes is in `SKILLS_WITH_HINTS`).
- **Phase 9 carve-out fidelity fix (Unicode arrow)**: executing-bug-fixes IS in the three-skill carve-out-fix set (`executing-chores`, `executing-bug-fixes`, `implementing-plan-phases`). The pre-change FR-14 carve-out bullet read `**FR-14 console echo lines** -- \`[model] step {N} ({skill}) -> {tier} (...)\` audit-trail lines emitted by \`prepare-fork.sh\`. The Unicode \`->\` is the documented emitter format; do not rewrite to ASCII.` — the trailing "Unicode `->`" was the self-contradictory form (text says "Unicode" but the rendered glyph is the ASCII `->`). Corrected both occurrences in the bullet (the `[model]` echo example and the trailing "Unicode `→`" reference) to the canonical Unicode `→` (U+2192) per FEAT-014's emitter format and FR-4's Correction note. The change is a carve-out fidelity fix, not a behavioral change — script-emitted FR-14 echo lines (`prepare-fork.sh`) are unchanged and continue to emit the canonical Unicode `→`. Called out explicitly in the commit message.
- **FR-7**: confirmed Phase 0 audit findings for `scripts/__tests__/executing-bug-fixes.test.ts` (28 assertions across 6 describe blocks: SKILL.md headings/literals/root-cause-driven-execution surface, allowed-tools frontmatter, PR template, workflow-details Phase 1/2/3 headings and reproduction-steps matcher, github-templates.md exclusion guards, validate-API). All assertions are heading / literal-phrase / frontmatter / template-existence / validate-API checks against load-bearing surface preserved by the compression. No test assertion updates required (the test file does not pin the "Unicode `->`" / "Unicode `→`" carve-out string).

**Per-axis outcomes (Phase 10, `documenting-qa`):**
- **FR-1**: lite-style prose compression applied to `# Documenting QA` intro (compressed `You operate as` → `Operate as`), `## When to Use This Skill` bullets (last bullet tightened from `... so you have real code diff and a user-facing PR summary to plan against` to `... real code diff and a user-facing PR summary are required`), `## Arguments` bullets (preserving the literal `When argument is provided` / `When no argument is provided` phrases verbatim per cross-skill `argument-hint.test.ts`), `## State File Management` (collapsed `At the start of this skill, create ... via the Write tool (empty content). This signals the stop hook that documenting-qa is the active skill.` → `At skill start, create ... via Write (empty content). This signals the stop hook that documenting-qa is active.`), `## Important: Bash-for-scripts-only` (compressed two-sentence pre-amble + closing into a single dense pair), `## Step 1` substeps 1–4 (tightened sub-clause prose; preserved `capability-discovery.sh` invocation block, JSON field list, and `mode: exploratory-only` literal verbatim), `## Step 2` (compressed precedence intros and the final Rationale paragraph from `The point of this prohibition: ...` → `Rationale: ...`; preserved the **Forbidden reads during planning** paragraph verbatim including the `Do NOT read \`requirements/...\``, `FR-3 covers X`, `FR-N` literals required by the `documenting-qa.test.ts` `Do NOT read\s+\`?requirements/` regex assertion and the stop-hook no-`FR-N` guard), `## Step 3` trailing sentence (`The diff + the user summary + the capability report are the only inputs you plan against.` → `Diff + user summary + capability report are the only planning inputs.`), `## Step 4` final paragraph (compressed persona-overlay description), `## Step 6` Sections bullet (`with all five dimension subheadings, each containing either scenario lines or a matching justification` → `all five dimension subheadings, each with either scenario lines or a matching justification`), `## Step 7` (`State in your last message that the plan file path is ...` → `State in the last message that the plan file path is ...`; the no-`FR-N`-in-`Scenarios` guard reference preserved verbatim), and the closing `> Note:` paragraph (compressed Option-B clause). The `## Output Style` section (lines 45-75) was preserved verbatim per FR-4 (FEAT-023 carve-out for main-context skills with the explicit fork-to-orchestrator-N/A clause).
- **FR-2**: no-op. No non-dispatcher section in SKILL.md exceeded ~25 lines of procedural narrative. The `## Output Style` section (~31 lines) is doctrine/rules (FR-4 preservation target), not procedural narrative subject to relocation. Steps 1–7 are each 5–14 lines after compression. The two existing `assets/test-plan-template-*.md` files are structural templates measured by FR-5 but excluded from FR-2 relocation (they are referenced via a markdown link from Step 6, not procedural narrative). **Edge Case 1 resolution**: this skill has no `references/` directory pre-change; FR-2 is a no-op so no `references/` directory was created post-change. **Edge Case 9 resolution (explicit before/after `references/`-absent state)**: the `references/`-absent state is preserved across this phase — pre-change measurement table contains no `references/` row for `documenting-qa`, post-change measurement table contains no `references/` row for `documenting-qa`, and the per-skill subtotal continues to cover only `SKILL.md` + `assets/test-plan-template-v2.md` + `assets/test-plan-template.md`.
- **FR-3**: no-op. SKILL.md contains one table (`## Relationship to Other Skills` nine-row task→approach matrix). The five **Dimensions** bullet list (Inputs / State transitions / Environment / Dependency failure / Cross-cutting) under Step 5 is a single-axis classification list, not a multi-row table, and is not repeated elsewhere in SKILL.md. No repetition or near-duplication exists. Nothing to collapse.
- **FR-4**: verified. All carve-outs intact: `## Output Style` section preserved verbatim including (a) `### Lite narration rules` (no-preamble, no-emoji, ASCII-arrows-in-skill-prose clause), (b) `### Load-bearing carve-outs` (error messages from `fail` calls including `capability-discovery.sh` / `persona-loader.sh` / `gh pr view` / `git diff` failures and the stop-hook block message; security-sensitive warnings; interactive prompts including the requirement-ID prompt, user-summary prompt, and "what to test" pointer prompt; **Findings display from `reviewing-requirements`** N/A bullet retained for cross-skill consistency; FR-14 echo lines `[model] step {N} ({skill}) -> {tier} (...)` with the `Typically not emitted here since this skill runs in main context, not forked, but retained for cross-skill consistency` qualifier; tagged structured logs `[info]` / `[warn]` / `[model]`; user-visible state transitions), (c) `### Fork-to-orchestrator return contract` (the explicit **main context** / **not** as an Agent fork clause, the `done | artifact=... | <note>` / `failed | <reason>` shapes do **not** apply clause, the no-subagent-boundary clarification, the Stop-hook structural-conformance enforcement reference at `scripts/stop-hook.sh`, and the no-`FR-N`-in-`Scenarios` guard reference), and (d) the `Precedence` note (carve-out wins over lite-narration rules). The forbidden-reads-during-planning rule (the `Do NOT read \`requirements/features/FEAT-*.md\`, \`requirements/chores/CHORE-*.md\`, or \`requirements/bugs/BUG-*.md\` beyond the isolated \`## User Story\` block.` paragraph + the `FR-3 covers X` self-detection example + the rationale paragraph) was preserved verbatim. The `FR-N`-in-Scenarios stop-hook guard is documented twice as required: once in Step 2 (`The stop hook verifies no \`FR-N\` references leak ...`), once in Step 7 (`... and the no-\`FR-N\`-in-\`Scenarios\` guard).`), and once in the Verification Checklist (`The \`## Scenarios (by dimension)\` section contains no \`FR-N\` references`). All FR-7 must-preserve items confirmed: `## When to Use This Skill`, `## Verification Checklist`, `## Relationship to Other Skills` headings; literal phrases `executing-qa`, `FEAT-`, `CHORE-`, `BUG-`, `requirements/features/`, `requirements/chores/`, `requirements/bugs/`, `qa/test-plans/QA-plan-`, `capability-discovery.sh`, `persona-loader.sh`, `test-plan-template-v2.md`, `version: 2`; the `not.toContain('qa-verifier')` exclusion guard (no `qa-verifier` mention reintroduced); v1 and v2 template heading assertions on the asset files (assets unchanged — `## Metadata`, `## Existing Test Verification`, `## New Test Analysis`, `## Coverage Gap Analysis`, `## Code Path Verification`, `## Plan Completeness Checklist` on v1; `## User Summary`, `## Capability Report`, `## Scenarios (by dimension)`, all five dimension subheadings, `## Non-applicable dimensions` on v2); frontmatter literals `type: command`, `not.toContain('type: prompt')`, the `${CLAUDE_PLUGIN_ROOT}/skills/documenting-qa/scripts/stop-hook.sh` Stop-hook command path with all required substring components (`hooks:`, `Stop:`, `type: command`, `command:`, `${CLAUDE_PLUGIN_ROOT}/skills/documenting-qa/scripts/stop-hook.sh`); `allowed-tools` frontmatter (`Bash` allowed for `capability-discovery.sh` / `persona-loader.sh` / `gh` / `git` invocation, `Agent` excluded per qa-verifier-Ralph-loop removal).
- **Cross-skill carve-out (confirmed)**: the `## Arguments` bullets retain `When argument is provided` / `When no argument is provided` verbatim — required by `scripts/__tests__/argument-hint.test.ts` (documenting-qa is in `SKILLS_WITH_HINTS`).
- **Phase 10 carve-out note (Unicode arrow)**: documenting-qa is NOT in the three-skill carve-out-fix set (`executing-chores`, `executing-bug-fixes`, `implementing-plan-phases`); the FR-14 line in this SKILL.md uses `Unicode \`->\`` as the documented emitter format (with the additional `Typically not emitted here since this skill runs in main context` qualifier) and is intentionally retained — the self-contradictory form correction is reserved for Phase 12.
- **Phase 10 carve-out note (main-context fork-to-orchestrator clause)**: the `### Fork-to-orchestrator return contract` subsection is preserved verbatim with its main-context-N/A clause that explicitly states the standard `done | artifact=... | <note>` / `failed | <reason>` shapes do NOT apply because `documenting-qa` runs in the orchestrator's main conversation (feature chain step 5; chore/bug chain step 3), not as an Agent fork — output flows directly to the user, not to a parent orchestrator. This is the same carve-out shape used by `documenting-features` / `documenting-chores` / `documenting-bugs` / `managing-work-items` (all main-context skills). Structural conformance of the emitted artifact is enforced by the Stop hook at `scripts/stop-hook.sh`, not by a subagent-return contract.
- **FR-7**: confirmed Phase 0 audit findings for `scripts/__tests__/documenting-qa.test.ts` (54 assertions across 6 describe blocks: SKILL.md frontmatter and content (15), allowed-tools frontmatter (3), stop-hook frontmatter (5), stop-hook behavioral artifact-grading via real `stop-hook.sh` invocation in tmpdir fixtures (12), v1 template content (6), v2 template content (8), validate-API (1)). All SKILL.md assertions are heading / literal-phrase / frontmatter / regex (`Do NOT read\s+\`?requirements/`) / not.toContain checks against load-bearing surface preserved by the compression. The behavioral stop-hook tests run the script binary in tmpdir fixtures and are unaffected by SKILL.md edits. No test assertion updates required.
...
| `executing-bug-fixes` | `SKILL.md` | 0 | 0.00% | -33 | -1.78% | -209 | -1.60% | -52 |
| `executing-bug-fixes` | `references/workflow-details.md` | 0 | 0.00% | 0 | 0.00% | 0 | 0.00% | — |
| `executing-bug-fixes` | `assets/pr-template.md` | 0 | 0.00% | 0 | 0.00% | 0 | 0.00% | — |
| `executing-bug-fixes` | **subtotal** | **0** | **0.00%** | **-33** | **-0.81%** | **-209** | **-0.76%** | **-52** |
| `documenting-qa` | `SKILL.md` | 0 | 0.00% | -73 | -3.70% | -372 | -2.71% | -92 |
| `documenting-qa` | `assets/test-plan-template-v2.md` | 0 | 0.00% | 0 | 0.00% | 0 | 0.00% | — |
| `documenting-qa` | `assets/test-plan-template.md` | 0 | 0.00% | 0 | 0.00% | 0 | 0.00% | — |
| `documenting-qa` | **subtotal** | **0** | **0.00%** | **-73** | **-2.68%** | **-372** | **-2.00%** | **-92** |
...
**Phase 9 delta notes:**
- SKILL.md line count is unchanged at 212 — compression replaced longer phrasings with shorter ones on the same logical lines (FR-2 / FR-3 were no-ops). The reduction shows in word count (-1.78%, -33 words) and char count (-1.60%, -209 chars / -52 chars/4 tokens, from 3263 to 3211).
- Magnitude is intentionally smaller than Phase 8 (`executing-chores`: -2.62% chars / -73 chars/4): `executing-bug-fixes` SKILL.md is already denser (212 lines vs 179), with several must-preserve literal-phrase clusters under `## Root Cause Driven Execution` (`Redeclare root causes`, `Address root causes systematically`, `Verify per root cause`) that bound the achievable compression. The Phase 0 baseline already reflected the FEAT-022 root-cause-driven-execution rewrite, leaving less surface to compress.
- The `## Output Style` section (FR-4 carve-out, lines 77-111, ~35 lines including the **PR creation is this skill's responsibility** carve-out — same exception as Phase 8 distinguishing executing-bug-fixes from `implementing-plan-phases`) was preserved verbatim — only the FR-14 carve-out bullet's "Unicode `→`" reference was corrected from the self-contradictory "Unicode `->`" form. The achievable reduction is bounded by the ~177 non-`## Output Style` lines plus the frontmatter.
- The Phase 9 carve-out fidelity fix (Unicode `->` → Unicode `→`) was applied within the FR-1 commit. Two occurrences corrected in the FR-14 bullet: the `[model]` echo example and the trailing "Unicode `→`" reference. This brings executing-bug-fixes to canonical-form parity with the FEAT-014 emitter format (`prepare-fork.sh` itself is unchanged and continues to emit Unicode `→`).
- `references/workflow-details.md` (338 lines, the Phase 1 / Phase 2 / Phase 3 detailed bug-fix workflow procedure — slightly larger than the chores sibling at 277 lines) and `assets/pr-template.md` (180 lines, the bug-fix PR description template with Root Cause(s) / How Each Root Cause Was Addressed traceability table / per-RC verification checklist) were not edited and contribute zero delta — both files are already in their respective directories with SKILL.md dispatching to them via inline pointers.
- The FR-2 dispatch pointer to `references/workflow-details.md` is at the end of the `## Workflow Checklist` section block (Edge Case 8 compliant — pointer at end of dispatcher, not the start) and re-listed under `## References`.
- Per-skill subtotal: `executing-bug-fixes` totals 730 lines / 4064 words / 27319 chars post-change vs 730 lines / 4097 words / 27528 chars baseline (-33 words, -209 chars, all on SKILL.md).
- The `## Arguments` bullets retain the literal phrases `When argument is provided` and `When no argument is provided` verbatim — applied prospectively from the Phase 3 cross-skill audit lesson (`scripts/__tests__/argument-hint.test.ts`); executing-bug-fixes is in `SKILLS_WITH_HINTS`.
- Phase 9 IS in the three-skill Unicode-arrow carve-out-fix set; the corrective fix has been applied here (sibling to Phase 8). The remaining correction is reserved for Phase 12 (`implementing-plan-phases`).
- Post-change `npm test -- executing-bug-fixes` passes (28/28); `npm run validate` passes (13/13 lwndev-sdlc skills).

**Phase 10 delta notes:**
- SKILL.md line count is unchanged at 200 — compression replaced longer phrasings with shorter ones on the same logical lines (FR-2 / FR-3 were no-ops). Word count drops 3.70% (-73 words) and char count drops 2.71% (-372 chars / -92 chars/4 tokens, from 3431 to 3339).
- The `## Output Style` section (FR-4 carve-out, lines 45-75, ~31 lines including the `### Lite narration rules`, `### Load-bearing carve-outs`, and `### Fork-to-orchestrator return contract` subsections) was preserved verbatim. The fork-to-orchestrator subsection contains the **main-context-N/A clause** (the standard `done | artifact=... | <note>` / `failed | <reason>` shapes do NOT apply because `documenting-qa` runs in main context, not as an Agent fork — output flows directly to the user) which is itself a load-bearing carve-out per Phase 10 task instructions.
- The achievable reduction is bounded by the ~169 non-`## Output Style` lines plus the frontmatter. Of those, the `## Step 5: Build the adversarial plan` (Dimensions list + scenario shape + execution-mode definitions, ~25 lines) and `## Step 6: Emit the version-2 plan artifact` (frontmatter field list + sections list, ~17 lines) are dense structural specifications that carry must-preserve literals (`test-plan-template-v2.md`, `version: 2`, `## User Summary`, `## Capability Report`, `## Scenarios (by dimension)`, `## Non-applicable dimensions`, all five dimension subheadings, the `[P0|P1|P2]` and `mode:` tag literals) and were not compressed beyond formatting.
- The **forbidden-reads-during-planning rule** (Step 2 paragraph: `Do NOT read \`requirements/features/FEAT-*.md\`, \`requirements/chores/CHORE-*.md\`, or \`requirements/bugs/BUG-*.md\` beyond the isolated \`## User Story\` block. ... if you find yourself writing \`FR-3 covers X\`, you are doing it wrong.`) was preserved verbatim per the Phase 10 task carve-out instructions. The `Do NOT read\s+\`?requirements/` regex assertion in `documenting-qa.test.ts` line 74 continues to match.
- The **`FR-N`-in-Scenarios stop-hook guard** is documented at three sites in SKILL.md (Step 2 prose, Step 7 verification flow, Verification Checklist final bullet) — all three preserved verbatim. The behavioral stop-hook tmpdir-fixture tests (12 tests covering well-formed plans, missing frontmatter, missing `version: 2`, missing Scenarios section, FR-N leak detection, empty dimensions, missing priority/mode tags, `stop_hook_active=true` skip path, state-file guard, malformed JSON, missing artifact, plan-file-resolution failure, FR-N tokens allowed in Non-applicable dimensions) are unaffected by SKILL.md edits — they exercise the script binary directly.
- **Edge Case 1 / Edge Case 9 resolution**: documenting-qa has no `references/` directory pre-change. FR-2 was a no-op for relocation, so no `references/` directory was created post-change. The `references/`-absent state is preserved across this phase. Per Edge Case 9, the per-skill subtotal continues to cover only `SKILL.md` + `assets/test-plan-template-v2.md` + `assets/test-plan-template.md` (no `references/` row) in both the Baseline Measurements and Post-Change Measurements tables.
- The two `assets/test-plan-template-*.md` files (`test-plan-template-v2.md` 65 lines / 334 words / 2252 chars; `test-plan-template.md` 65 lines / 422 words / 2575 chars) were not edited and contribute zero delta — they are structural templates measured for FR-5 baseline parity but excluded from FR-1 / FR-2 / FR-3 (they are referenced via a markdown link from Step 6, not procedural narrative subject to compression or relocation, and their headings are pinned by the v1/v2 template-test assertions).
- Per-skill subtotal: `documenting-qa` totals 330 lines / 2654 words / 18181 chars post-change vs 330 lines / 2727 words / 18553 chars baseline (-73 words, -372 chars, all on SKILL.md).
- The `personas/` and `scripts/` directories under `documenting-qa` (the `qa.md` persona overlay and the three scripts: `capability-discovery.sh`, `persona-loader.sh`, `stop-hook.sh`) remain out of scope for this rollout per Phase 0 step 2.
- The `## Arguments` bullets retain the literal phrases `When argument is provided` and `When no argument is provided` verbatim — required by `scripts/__tests__/argument-hint.test.ts` (documenting-qa is in `SKILLS_WITH_HINTS`).
- Phase 10 is NOT in the three-skill Unicode-arrow carve-out-fix set; the documented `Unicode \`->\`` form in the FR-14 carve-out bullet is the canonical FR-14 emitter format for this skill (with the additional `Typically not emitted here since this skill runs in main context` qualifier) and is intentionally retained per the FEAT-023 install. The corrective fix is reserved for Phase 12 (`implementing-plan-phases`).
- Post-change `npm test -- documenting-qa` passes (54/54 across 1 file); cross-skill `npm test -- argument-hint` passes (56/56); `npm run validate` passes (13/13 lwndev-sdlc skills).
