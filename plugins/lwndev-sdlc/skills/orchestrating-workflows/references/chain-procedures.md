...
1. Run `bash "${CLAUDE_PLUGIN_ROOT}/skills/orchestrating-workflows/scripts/check-resume-preconditions.sh" <ID>`.
   ...
   - **paused** + `qa-loop-exhausted` → Parse resume flags from the user's invocation argument via `parse-model-flags.sh`. Two branches (mutually exclusive; both together exit 2 with the FR-8 verbatim message per Phase 1):
     - `--approve-advance`: call `workflow-state.sh resume {ID}`; advance past the QA step with `qaFixAttempts` preserved (counter NOT reset — the history is traceable via stateEvents). Continue to the next step.
     - `--qa-loop-cap <N>`: call `workflow-state.sh set-qa-loop-cap {ID} <N>`; call `workflow-state.sh reset-qa-fix-attempts {ID}`; call `workflow-state.sh resume {ID}`; re-enter the QA dispatch loop (re-invoke `executing-qa` in re-QA mode, then run `qa-dispatch.sh` again).
     - Neither flag: inform the user the workflow remains paused. Suggest `--approve-advance` or `--qa-loop-cap <N>`.
   - **paused** + `qa-error` → Surface the underlying QA ERROR verdict reason from the artifact at `qa/test-results/QA-results-{ID}.md`. User resolves manually (e.g., fix the test environment) then re-invokes the orchestrator with the same ID. On re-invoke: call `workflow-state.sh resume {ID}`; re-enter the QA step (re-QA mode if re-QA baseline exists, otherwise initial run).
   - **paused** + `fix-suite-failed` → Surface the FR-4 fix-phase failure reason. v1 has no auto-revert (Edge Case 13). User resolves manually (reverts or fixes the suite failure), then re-invokes. On re-invoke: call `workflow-state.sh resume {ID}`; re-enter the QA dispatch loop from the `fix-phase` branch.
   - **paused** + `adoption-failed` → Surface the FR-5 partial-success reason: the failing QA test path and the `<N> adopted, <M> remaining` count. `adoptedTests` is preserved across the pause per FR-4 step 2.3. User resolves the failing adoption manually (e.g., fix the import structure in the QA test), then re-invokes. On re-invoke: call `workflow-state.sh resume {ID}`; re-enter the adopt phase of `addressing-qa-findings`.
