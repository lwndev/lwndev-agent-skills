...
- **Workflow-defined approval gates override "work without stopping"** -- the system reminder phrase "work without stopping for clarifying questions" applies to ad-hoc clarifying questions, NOT to workflow-defined approval gates. The gates `plan-approval`, `pr-review`, `findings-decision`, `review-findings`, and `merge-approval` always require explicit user confirmation (a UserPromptSubmit event or a Hook A-recorded approval marker), regardless of any "work without stopping" instruction in the context. Do not interpret that reminder as authorization to self-resume, self-clear-gate, or self-approve at any of these five gates. See `references/step-execution-details.md#pause-steps` for the halt-and-surface procedure at the `plan-approval` / `pr-review` pause sites and `references/step-execution-details.md` Finalize fork-step blocks for the `merge-approval` gate procedure.
