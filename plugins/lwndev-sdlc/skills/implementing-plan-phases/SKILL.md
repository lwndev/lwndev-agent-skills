...
7. Verify deliverables: `bash "$SCRIPTS/verify-phase-deliverables.sh" "<plan-path>" <phase-N>`. Extracts backticked paths from the phase's `#### Deliverables`, checks existence, then runs `npm test`, `npm run build`, and (when the plan mentions `coverage` or a `[0-9]+%` threshold) `npm run test:coverage`. JSON stdout: `{"files":{"ok":[...],"missing":[...]},"test":"pass|fail|skipped","build":"...","coverage":"...","output":{...}}`. Exit `0` only when `files.missing` is empty AND every check is `pass` or `skipped`; else `1`. Gracefully degrades when `npm` is absent (`[warn]` to stderr; all three reported `skipped`).
8. Commit and push — always, no confirmation prompt: `bash "$SCRIPTS/commit-and-push-phase.sh" "<FEAT-NNN>" <phase-N> "<phase-name>"`. Produces canonical commit `<type>(<ID>): complete phase <N> - <name>` (`FEAT-`→`feat`, `CHORE-`→`chore`, `BUG-`→`fix`). Runs `git add -A` + commit, detects upstream, pushes (`-u origin <branch>` on first push; bare `git push` after). Stdout `pushed <branch>`. Exit `1` on `no changes to commit`, hook rejection, or push failure (git stderr verbatim + `[error] push failed; see Push Failure Recovery in SKILL.md`). Exit `2` on malformed args.

   **Push Failure Recovery.** On push rejection (remote has new commits), resolve directly with `git fetch origin && git rebase origin/<branch> && git push`. **Do not re-run `commit-and-push-phase.sh` after resolving** — by that point the tree is clean and the script's `git status --porcelain=v1` sanity gate trips `error: no changes to commit` and exits `1`, masking the successful recovery. Run the raw `git push` once the rebase lands. On auth failures, re-authenticate (e.g. `gh auth login`) and retry `git push` directly.
...
10. **After all phases complete:** Create pull request — gate on `verify-all-phases-complete.sh`, then invoke `create-pr.sh` (MUST include `Closes #N` if issue exists):
    ...
    `verify-all-phases-complete.sh`: exit `0` with `all phases complete` only when every phase is `✅ Complete`; otherwise `1` with JSON `{"incomplete":[{"phase":<N>,...}...]}` — treat any non-zero as "do not create PR". `create-pr.sh`: reads current branch, runs `git push -u origin <branch>`, assembles title `feat(<FEAT-NNN>): <summary>`, substitutes `scripts/assets/pr-body.tmpl`, runs `gh pr create`. Pass `--closes #N` when a GitHub issue exists. Exit `0` PR URL on stdout; `1` on git/gh failure; `2` on malformed args.
