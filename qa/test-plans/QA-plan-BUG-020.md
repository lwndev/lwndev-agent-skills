...
- [P0] After `set-gate <ID> merge-approval`, calling `advance <ID>` clears `.gate` to `null` and `.gateSetAt` to `null` as a side-effect of the inline mutation at `cmd_advance:1209-1210`; subsequent stop-hook fires (now with `.gate == null`) revert to the in-progress nudge path | mode: test-framework | expected: bats asserts post-advance state file has gate/gateSetAt = null
